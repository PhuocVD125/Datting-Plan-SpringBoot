...
const tagName = url.substring(url.lastIndexOf('/') + 1); // Extracts everything after the last '/'
console.log(tagName); // Outputs: "a"
const recomendationApi = "/api/v1/post/tag/"+tagName;
async function fetchPosts(page = 0) {
  const response = await fetch(recomendationApi+`?page=${page}`);
